...
RSpec.describe BattingStat, :type => :model do
	...
	describe ".import_from_csv" do
		...
		let(:csv_data_with_missing_headers){"spec/fixtures/missing_headers.csv"}
		let(:batting_stats){"spec/fixtures/7_batting_stats.csv"}

		it "throws exception when csv has missing headers" do
			expect{described_class.import_from_csv(csv_data_with_missing_headers)}.to raise_error(RuntimeError, "unknown column name- leagueee")
		end

		it "loads data given valid csv file" do
			result = described_class.import_from_csv(batting_stats)
			puts "#{result[:created].inspect}"
			expect(described_class.all.count).to be(7)
		end

	end

	describe "batting stat" do
